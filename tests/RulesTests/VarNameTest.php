...
use function Psrlint\Linter\createFixer;
...
class VarNamingTest extends TestCase
{
    ...
    public function testFixer()
    {
        ...
        $fixer($results, 'traverse.end');

        $this->assertEquals('camelCaps', $nodes[0]->name);
        $this->assertEquals('underscore', $nodes[1]->name);
        $this->assertEquals('snakeCase', $nodes[2]->name);
    }
}
